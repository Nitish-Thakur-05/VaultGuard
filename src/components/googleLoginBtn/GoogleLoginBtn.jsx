...
const GoogleLoginBtn = () => {
  ...
  async function googleLogin(e) {
    ...
    try {
      ...
      // database setup
      try {
        // seting data to database if data dont exists
        if (
          !(await getDoc(doc(db, "userData", auth.currentUser.uid))).exists()
        ) {
          await setDoc(
        ...
        toast.error("unable to set database");
        await signOut(auth);
      }
    } catch (error) {
      if (error.message === "Firebase: Error (auth/user-disabled).") {
        ...
        toast.error("Unable to login using google service now");
      }
    } finally {
